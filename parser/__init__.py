...
LABELS = [
    "amendment_amount",
    "amendment_amount_padding",
    "contract_amount",
    "contract_amount_padding",
    "other_amount",
    "other_amount_padding",
    "amount_alphabetic",
    "skip",
    "post_amended_amount",
    "post_amended_amount_padding"
]
...
MODEL_FILE = 'learned_settings.crfsuite'  # filename for the crfsuite settings
# *************************
...
try:
    ...
    TAGGER = None
    warnings.warn((
        'You must train the model (parserator train [traindata] ' +
        '[modulename]) to create the {} file before you can use the parse ' +
        'and tag methods').format(MODEL_FILE)
    )
...
def parse(raw_string):
    if not TAGGER:
        raise IOError((
            '\nMISSING MODEL FILE: {}\nYou must train the model before you ' +
            'can use the parse and tag methods\nTo train the model and ' +
            'create the model file, run:\nparserator train [traindata] ' +
            '[modulename]').format(MODEL_FILE)
        )
...
def tokenize(raw_string, offsets=False):
    if not offsets:  # standard run of tokenization
        ...
        output = []
        for i in re.finditer('\$[\d]+(\.\d)? billion|\w+|\$[\d\,]+(.\d\d)?',
                             raw_string):
...
def tokens2features(tokens):
    ...
    for token in tokens[1:]:
        ...
        # for example, a feature for whether a certain character
        # has appeared previously in the token sequence

        feature_sequence.append(token_features)
        previous_features = current_features

    if len(feature_sequence) > 1:
        # Features for the tokens at the beginning and end of a string:
...
# define any other methods for features.
# this is an example to get the casing of a token
