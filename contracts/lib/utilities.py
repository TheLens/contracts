...
"""
Utility functions for gathering and organizing
...
class Utilities(object):
    ...
    def scan_index_page(self, page_no):
        '''
        ...
        html = self.get_individual_contract_page(page_no)
        output = self.get_purchase_order_numbers_from_index_page(html)
        for purchase_order_number in output:
            self.check_if_need_to_download_contract(purchase_order_number)
            time.sleep(10)

    @staticmethod
    def check_if_invalid(html):
        """
    ...
    @staticmethod
    def check_if_need_to_download_contract(purchase_order_number):
        log.info(
            'Daily scraper found purchase order %s', purchase_order_number)
        try:
    ...
    @staticmethod
    def get_purchase_order_numbers_from_index_page(html):
        '''
    ...
    @staticmethod
    def get_individual_contract_page(pageno):
        '''
